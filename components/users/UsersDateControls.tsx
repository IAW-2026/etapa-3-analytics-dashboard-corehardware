...
// Mismo discriminated union que OrdersSummaryDateControls: TypeScript
// obliga a pasar dateInfo solo con "day" y rangeInfo solo con "month"/"year".
type UsersDateControlsProps =
  | { granularity: "day"; dateInfo: DateFallbackInfo }
  | { granularity: "month" | "year"; rangeInfo: RangeInfo };

// ÚNICA diferencia real contra OrdersSummaryDateControls: el router.push
// apunta a /usuarios en vez de /pedidos/resumen. Si la ruta real de esta
// pantalla no es exactamente "/usuarios" (por ejemplo si vive bajo otro
// segmento dentro de (protected)), hay que ajustar la constante de abajo.
const BASE_PATH = "/usuarios";

export function UsersDateControls(props: UsersDateControlsProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* A diferencia de Pedidos-Resumen, acá "day" nunca cae en fallback:
          las queries de growth siempre devuelven una fila para el día
          pedido (real o en cero), así que este bloque de warning debería
          quedar siempre inactivo — se deja por compatibilidad de tipos
          con DateFallbackInfo, no porque se espere que dispare. */}
