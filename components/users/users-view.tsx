...
const TabButton = memo(function TabButton({ label, active, onClick }: TabButtonProps) {
    return (
        <button
            ...
            onClick={onClick}
            className={`px-3 py-1.5 text-xs font-mono rounded-md border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${active
        ...
        <thead>
            <tr className="border-b border-zinc-800">
                {headers.map((h) => (
                    <th
                        key={h.label}
                        className={`text-left px-4 py-3 text-xs font-mono tracking-[0.1em] uppercase text-zinc-500 ${h.hideBelowLg ? "hidden lg:table-cell" : ""
                            }`}
                    >
                        {h.label}
                    </th>
...
const BUYERS_HEADERS: { label: string; hideBelowLg?: boolean }[] = [
    ...
    { label: "Fecha alta", hideBelowLg: true },
];
                ...
                {buyers.map((b) => (
                    <EntityMobileCard
                        ...
                        rows={[
                            ...
                            { label: "Fecha alta", value: formatDate(b.fecha_alta) },
                        ]}
                    />
                ))}
            </div>
                ...
                {sellers.map((s) => (
                    <EntityMobileCard
                        ...
                        rows={[
                            ...
                            { label: "Fecha alta", value: formatDate(s.fecha_alta) },
                        ]}
                    />
                ))}
            </div>
                ...
                {operators.map((o) => (
                    <EntityMobileCard
                        ...
                        rows={[
                            ...
                            { label: "Fecha alta", value: formatDate(o.fecha_alta) },
